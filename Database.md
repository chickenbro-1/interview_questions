...
### [主从复制原理是什么？Replication](#主从复制原理是什么)
...
### [什么情况下会出现主从延迟？如何尽量减少延迟？](#什么情况下会出现主从延迟-如何尽量减少延迟)
...
![水平分库](/assets/horizontal-slicing-database-D4Brw-dN.png)
...
![分表](/assets/two-forms-of-sub-table-BBf2p_ED.png)
...
* **哈希分片**：求指定分片键的哈希，然后根据哈希值确定数据应被放置在哪个表中。哈希分片比较适合随机读写的场景，不太适合经常需要范围查询的场景。哈希分片可以使每个表的数据分布相对均匀，但对动态伸缩（例如新增一个表或者库）不友好。**比如新增或删除节点时，取模哈希会导致大量数据迁移,新增节点后，映射关系彻底改变, 缓存失效.**
...
Consistent hashing maps both data (keys) and nodes (servers) to a circular space (called a hash ring).
...
The key should cover most query scenarios to minimize the number of shards involved in a single query, reducing database load.
* 具有离散性，即能够将数据均匀地分散到各个分片上，避免数据倾斜和热点问题；
  The key should distribute data evenly across shards to avoid data skew and hotspot issues.
* 具有稳定性，即分片键的值不会发生变化，避免数据迁移和一致性问题；
The values of the sharding key should remain unchanged to prevent data migration and consistency problems.
* 具有扩展性，即能够支持分片的动态增加和减少，避免数据重新分片的开销。
The key should support dynamic shard expansion and reduction to avoid the overhead of redistributing data.
